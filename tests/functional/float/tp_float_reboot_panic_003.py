...
#
# ID:
#       functional/float/tp_float_reboot_panic_003
#
# DESCRIPTION:
#       Reboot one root domain and another simutaneously, IO domain with two VFs
#       configured with Multipath is alive during the period and the two VFs
#       with IO workload is still ONLINE after the period.
#
# STRATEGY:
...
#   	- Reboot root domain NPRD1 by 'reboot' and panic NPRD2 by
#         'echo "rootdir/W 0"|mdb -kw'.
#   	- After interruption, check VFs state by "hotplug list" in IO domain,
#         both should be "ONLINE", check the logical path from VF, should be
#         seen. Check the IO workload,should be zero.
#
...
def tp_float_reboot_panic_003():

    common.info_print_report("FC-IOR functional test06 TP3: reboot_panic")
