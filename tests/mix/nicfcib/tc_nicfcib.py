...
# test purposes
from tp_nicfcib_001 import tp_nicfcib_001
from tp_nicfcib_002 import tp_nicfcib_002


def startup():

    info_print_report("FC-IOR mix nicfcib:  startup")
    ...
    nic_remote_host_dict = {nic_remote_host: {"password": nic_remote_password,
    ...
    root_dict = {nprd1_name: {"password": nprd1_password,
                 ...
                 nprd2_name: {"password": nprd2_password,
                              "pf": nic_pf_2}}
    try:
        check_nic_pf_be_connected(nic_remote_host_dict,
                                  root_dict)
    ...
    vfs_list = [a_vf, b_vf, nic_a_vf, nic_b_vf, ib_a_vf, ib_b_vf]
    for vf in vfs_list:
        try:
            info_print_report(
    ...
    # run io traffic on nic ipmp interface, io traffic is just ping.
    try:
        info_print_report(
            "Run traffic between remote NIC interface and "
            "NIC ipmp group in io domain [%s]" % iod_name)
        ...
        error_report(ctiutils.cti_traceback())
        ctiutils.cti_deleteall(
            "Failed to run traffic between remote NIC interface"
            " and NIC ipmp in io domain [%s]" % iod_name)
        return 1

    # run io traffic on ib ipmp interface, io traffic is just ping.
    try:
        info_print_report(
            "Run traffic between remote IB link and "
            "IB ipmp group in io domain [%s]" % iod_name)
        run_ping_traffic_in_domain(
            iod_name,
            iod_password,
            ib_ip_addr,
...
def cleanup():
    ...
    info_print_report("FC-IOR mix nicfcib:  cleanup")
...
#
test_list = {}
test_list[1] = tp_nicfcib_001
test_list[2] = tp_nicfcib_002
