...
#
# ID:
#       mix/mix_nicfcib/tp_mix_nic_fc_ib_001
#
# DESCRIPTION:
...
#       is alive during the reboot period.
#       Besides, the two FC VFs where running IO workload are still ONLINE and
#       io workload continues after reboot. The two NIC vfs where running IO
#       workload are still ONLINE and io workload continues after reboot. The
#       two IB VFs where running io workload are still ONLINE and io workload
#       are continues after reboot.
#
# STRATEGY:
...
#         the alternative path. Check VF created from NPRD2 by "hotplug list" in
#         IO domain, should be "ONLINE".
#       - During reboot, check IB VF from NPRD1 by "hotplug list" in IO domain,
...
def tp_nicfcib_001():
    ...
    common.info_print_report("FC-IOR mix nicfcib TP1: reboot")
